...
impl ActorBuilder {
    ...
    fn with_name<'a>(&'a mut self, name: &'a str) -> &'a mut Self {
        self.name = Some(name.to_string());
        self
    }

    fn with_player(&mut self, is_player: bool) -> &mut Self {
        self.is_player = is_player;
        self
    }

    fn with_ethereal(&mut self, is_ethereal: bool) -> &mut Self {
    ...
    fn build(
        ...
        world: &mut World,
        f: &dyn Fn() -> Box<dyn occupation::Occupation>,
    ) {
        let names = vec![
...
fn main() {
    ...
    //
    {
        ...
        for _ in 0..2 {
            ActorBuilder::new().build(&mut rng, &mut world, &|| Box::new(HouseBuilder::new()));
        }
        for _ in 0..2 {
            ActorBuilder::new().build(&mut rng, &mut world, &|| Box::new(Farmer::new()));
        }
        for _ in 0..20 {
