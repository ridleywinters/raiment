...
fn add_voxel(group: &mut SceneNode, x: f32, height: f32, z: f32) -> kiss3d::scene::SceneNode {
    let s = 0.5f32;
    let e = -s;
    let y = e + (0.0 + height);
...
fn sync_world_map(wmg: &mut WorldMapGeometry, window: &mut Window, world: &World) {
    ...
    for y in 0..REGION_SIZE {
        for x in 0..REGION_SIZE {
            ...
            // coordinate system
            let mut c = add_voxel(&mut group, x as f32, z as f32, y as f32);
...
fn main() {
    ...
    while window.render_with_camera(&mut camera) {
        ...
        sync_world_map(&mut wmg, &mut window, &world);
