...
/// Filter parameters for a texture.
...
pub enum TextureFilter {
    ...
    /// specified texture coordinates. These can include items wrapped or repeated from other
    ///  parts of a texture, depending on the values of GL_TEXTURE_WRAP_S and GL_TEXTURE_WRAP_T,
...
impl Into<u32> for TextureFilter {
    #[inline]
    fn into(self) -> u32 {
        match self {
            TextureFilter::Nearest => Context::NEAREST,
            TextureFilter::Linear => Context::LINEAR,
            TextureFilter::LinearMipmapLinear => Context::LINEAR_MIPMAP_LINEAR,
        }
    }
}
...
impl Texture {
    ...
    pub fn set_minification_filter(&mut self, filter: TextureFilter) {
        let ctxt = Context::get();
        verify!(ctxt.bind_texture(Context::TEXTURE_2D, Some(&self)));
        let filter: u32 = filter.into();
        verify!(ctxt.tex_parameteri(Context::TEXTURE_2D, Context::TEXTURE_MIN_FILTER, filter as i32));
    }

    
    /// Sets the magnification filter for the texture
    pub fn set_magnification_filter(&mut self, filter: TextureFilter) {
        let ctxt = Context::get();
        verify!(ctxt.bind_texture(Context::TEXTURE_2D, Some(&self)));
        let filter: u32 = filter.into();
        verify!(ctxt.tex_parameteri(Context::TEXTURE_2D, Context::TEXTURE_MAG_FILTER, filter as i32));
    }
}
